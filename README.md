...
## Formalism
The angular distribution $W(\theta)$ is the distribution of gamma rays emitted, not the distribution observed. The number of gamma rays emitted can be found from the number observed through the angular-depedent observation efficiency $\epsilon(E,\theta_i)$, which is the efficiency of observing the lab frame gamma-ray energy in the polar angle cut at $\theta_i$. This efficiency is calculated according to the following. The total GRETINA singles efficiency for a gamma ray that is Doppler shifted according to angle $\theta_i$ is $\epsilon\left(E \gamma (1-\beta\cos\theta_i)\right)$ where $\epsilon(E)$ is given by $\epsilon(E) = \frac{N_C}{32} 4.532 \left(100 + E\text{[keV]} \right)^{\left( -0.621 \right)}$ where $N_C$ is the number of GRETINA crystals used in your experiment. The total efficiency at the Doppler-shifted energy is then scaled according to the fractional angular coverage in GRETINA of the angle bin. The fractional angular coverage can be found by sorting source data according to the same polar angle cuts and obtaining the full energy peak counts of a transition within each angle cut. The 1408-keV peak from ${}^{152}\mathrm{Eu}$ was used to obtain the fractional angular coverage for GRETINA. The efficiency $\epsilon(E,\theta)$ can be expressed as:

$\epsilon(E,\theta_i) = \epsilon(E\gamma(1-\beta\cos\theta_i)) \frac{N^{1408}_i}{\sum_j{N^{1408}_j}}$ 

where $N^{1408}_i$ is the number of 1408-keV ${}^{152}\mathrm{Eu}$ source counts within the angle bin $\theta_i$. 

Each angular distribution data point is then normalized to a lab-frame isotropic angular distribution, which is equivalent to dividing by the factor $(1-\beta^2)/(1-\beta\cos\theta_i)^2$ up to a constant. Given that GRETINA has an angular coverage less than $4\pi$, the overall number of observed gamma rays depends on the shape of the angular distribution within the angular coverage. The constant is determined such that the final normalized angular distribution data points have an average of 1. The fit function is also normalized by the same factor. This normalization is useful because it eliminates the need for a scaling parameter in the angular distribution fit function.

The function `adRatios()` in `modules/angular_analyze.py` transforms the angle-bin counts into gamma-ray intensities that are normalized to the lab-frame isotropic angular distribution.  